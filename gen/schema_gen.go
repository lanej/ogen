...
func (g *schemaGen) nonPrimitiveObjectEnum(name string, schema *jsonschema.Schema) (*ir.Type, error) {
	...
	if discriminatorField != "" {
		...
		// Note: We only need ValueDiscriminators, not UniqueFields
		// The ir/json.go Sum() function will return SumJSONFields when ValueDiscriminators is set
	} else {
		// No discriminator field found, use type-based discrimination as fallback
		// This handles cases where variants have different field structures
